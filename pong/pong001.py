...
# Assignment says randrange(120, 240) px/sec., that is (2, 4) @60Hz.
...
def draw(canvas):
    ...
    # Render scores earlier than the template so the ball is "above" them
    ...
    # Reverse horizontal direction and increase velocity after hitting a paddle.
...
# If we don't, the paddle will go off the screen if we press the same key again after it has hit the floor or ceiling.
def keydown(key):
...
resetButton = frame.add_button("Reset", reset, 150)
# Start frame
new_game()
frame.start()
